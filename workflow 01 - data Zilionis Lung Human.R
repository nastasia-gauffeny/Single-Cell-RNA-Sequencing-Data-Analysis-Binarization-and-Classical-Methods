...
BP_count <- ZilionisLungData('human', filter = TRUE)
...
BP_count <- AddMetaData(BP_count, BP_count@meta.data$Patient, col.name = "group")
...
BP_samplesheet <- data.frame(celltype = as.factor(BP_count@meta.data$Major.cell.type),
                             ...
                             group = as.factor(BP_count@meta.data$Patient),
...
plot_Label <- function(df = BP_set$samplesheet, dataset = "", max_x = 40) {
  ...
  plot <- pie_data |>
    ...
    theme(panel.background = element_blank(),
          ...
          legend.key.size = unit(0.06, "inch"),
...
plot_Label(dataset = "Zilionis Lung Human")
