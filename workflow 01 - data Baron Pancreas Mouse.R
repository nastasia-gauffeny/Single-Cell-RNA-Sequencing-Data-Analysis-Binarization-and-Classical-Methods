...
BP_count <- BaronPancreasData('mouse') # donor = strain (2 levels)
...
BP_count <- AddMetaData(BP_count, BP_count@meta.data$strain, col.name = "group")
...
BP_samplesheet <- data.frame(celltype = as.factor(BP_count@meta.data$label),
                             colnames = as.factor(colnames(BP_count)),
...
plot_Label(dataset = "Baron Pancreas Mouse", max_x = 60)
